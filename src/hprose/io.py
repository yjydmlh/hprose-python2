...
class HproseWriter(object):
    ...
    def writeBytesWithRef(self, bytes):
        if not self.refer.write(bytes): self.writeBytes(bytes)
    ...
    def writeStringWithRef(self, str):
        if not self.refer.write(str): self.writeString(str)
    ...
    def writeListWithRef(self, list):
        if not self.refer.write(list): self.writeList(list)
    ...
    def writeMapWithRef(self, map):
        if not self.refer.write(map): self.writeMap(map)
